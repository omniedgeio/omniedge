...
function App() {
  ...
  // Periodically refresh device list (including online status) for expanded networks
  useEffect(() => {
    if (!isLoggedIn) return;

    const deviceRefreshInterval = setInterval(async () => {
      // Refresh devices for all expanded networks
      const expandedNetworkIds = Object.entries(expandedNetworks)
        .filter(([_, isExpanded]) => isExpanded)
        .map(([networkId]) => networkId);

      for (const networkId of expandedNetworkIds) {
        try {
          const devs: any = await invoke('get_network_devices', { networkId });
          setNetworkDevices(prev => ({ ...prev, [networkId]: devs || [] }));
        } catch (err) {
          console.error(`Failed to refresh devices for network ${networkId}:`, err);
        }
      }
    }, 10000); // Refresh every 10 seconds

    return () => clearInterval(deviceRefreshInterval);
  }, [isLoggedIn, expandedNetworks]);
  ...
  if (showDebug) {
    return (
      <div className="app debug-mode" ref={appRef}>
                ...
                <div className="this-device-card">
                  ...
                  <div className="card-content">
                    <div className="card-top-row">
                      ...
                      <span
                        className={`connection-status-badge ${status === 'connected' ? 'connected' : status === 'connecting' ? 'connecting' : 'disconnected'
                          }`}
                      >
